...
namespace
{
const uint16_t WEATHER_HTTP_TIMEOUT =
  2000;
...
portMUX_TYPE weatherMux =
  portMUX_INITIALIZER_UNLOCKED;

volatile bool weatherRequestRunning =
  false;

unsigned long lastWeatherRequest =
  0;

uint32_t lastDrawnWeatherRevision =
  UINT32_MAX;
...
)
{
  ...
  if (
    condition[0] == '\0' ||
    strcmp(condition, "unknown") == 0 ||
    strcmp(condition, "unavailable") == 0 ||
    doc["attributes"]["temperature"].isNull()
  )
  {
    return false;
  }
  ...
  copyText(
    ...
    sizeof(result.currentCondition),
    condition
  );

  result.currentTemperature =
    doc["attributes"]["temperature"]
      .as<float>();
...
)
{
  ...
  copyText(
    result,
    resultSize,
    weekdays[forecastTime.tm_wday]
  );
}

bool readForecastDay(
  JsonObject forecast,
  ForecastDay& result
)
{
  ...
  weekdayFromDate(
    datetime,
    result.weekday,
    sizeof(result.weekday)
  );

  copyText(
    result.condition,
    sizeof(result.condition),
    condition
  );

  result.highTemperature =
    forecast["temperature"].as<float>();

  result.lowTemperature =
    forecast["templow"].as<float>();

  return true;
}

bool loadWeatherForecast(
  WeatherData& result
)
{
  HTTPClient http;
  ...
  bool haveTargetDates =
    dateKey(1, targetDates[0], sizeof(targetDates[0])) &&
  ...
  if (haveTargetDates)
  {
    for (JsonObject forecast : forecasts)
    {
      ...
      for (int i = 0; i < 2; i++)
      {
        if (
          !found[i] &&
          strncmp(datetime, targetDates[i], 10) == 0
        )
        {
          found[i] =
            readForecastDay(
...
)
{
  ...
  else if (conditionIs(condition, "cloudy"))
  {
    ...
    conditionIs(condition, "rainy") ||
    conditionIs(condition, "pouring")
  )
  {
    drawRain(
      x,
    ...
    conditionIs(condition, "lightning") ||
    conditionIs(condition, "lightning-rainy")
  )
  {
    drawCloud(x, y - 5, C_GREY);
    tft.fillTriangle(
    ...
    conditionIs(condition, "snowy") ||
    conditionIs(condition, "snowy-rainy")
  )
  {
    drawSnow(x, y);
  }
  ...
  else if (conditionIs(condition, "hail"))
  {
    ...
    conditionIs(condition, "windy") ||
    conditionIs(condition, "windy-variant")
  )
  {
    drawWind(x, y);
  }
...
void initializeWeather()
{
  ...
  lastWeatherRequest =
    millis() -
    appConfig.weatherRefreshIntervalMs;
...
void maintainWeather()
{
  if (
    ...
    !hasValidHomeAssistantConfig(appConfig) ||
    appConfig.weatherEntity[0] == '\0' ||
    millis() - lastWeatherRequest <
      appConfig.weatherRefreshIntervalMs
  )
  {
    return;
...
bool weatherNeedsRedraw()
{
  ...
  return
    idleWeatherDirty ||
    revision != lastDrawnWeatherRevision;
}
...
void drawWeatherFields()
{
  ...
  drawColumnHeading(
    shown.forecast[0].valid
      ? shown.forecast[0].weekday
      : "--",
    centers[1]
  );
  drawColumnHeading(
    shown.forecast[1].valid
      ? shown.forecast[1].weekday
      : "--",
    centers[2]
  );
  ...
  if (shown.currentValid)
  {
    drawWeatherIcon(
      centers[0],
      119,
      shown.currentCondition
    );

    String currentTemperature =
      String((int)round(shown.currentTemperature));
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    drawWeatherIcon(
      centers[i + 1],
      119,
      shown.forecast[i].condition
    );

    String temperatures =
      String((int)round(shown.forecast[i].highTemperature));

    temperatures += "/";
    temperatures +=
      String((int)round(shown.forecast[i].lowTemperature));
...
String getWeatherStatus()
{
  ...
  bool forecastValid =
    status.forecast[0].valid &&
    status.forecast[1].valid;
